...
export interface PlacesState { //Solo me define como quiero que luzca ese objeto de PlacesState
  isLoading: boolean;
  userLocation?: [number, number]
}

const INITAL_STATE: PlacesState = { //Estado es la informacion que almacenamos en memoria
...
export const PlacesProvider = ({ children }: Props) => {
  ...
  useEffect(() => { //Obtener la geolocalizacion de la persona
    getUserLocation().then( LngLat => dispatch({ type: 'setUserLocation', payload: LngLat}) ) //Promise, en el then las coords
  }, [])
